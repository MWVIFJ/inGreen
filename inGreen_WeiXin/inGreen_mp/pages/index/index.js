...
const hefengKey = "a3c43ccd0ebd488984c03d16fe83031d"; //和风天气Web api的key
const mqttHost = "broker.emqx.io"; //mqtt服务器域名
const mqttPort = 8084; //mqtt服务器端口号
const hefengVIP = false; //和风天气是免费api还是付费api
const deviceSubTopic = "/inGreenTopic/sub"; //  设备订阅topic（小程序发布命令topic）
const devicePubTopic = "/inGreenTopic/pub"; //  设备发布topic（小程序订阅数据topic）
const mpSubTopic = devicePubTopic;
const mpPubTopic = deviceSubTopic;
const mqttUrl = `wxs://${mqttHost}:${mqttPort}/mqtt`; //mqtt连接路径
const hefengApi = "https://api.qweather.com/v7"; //付费API前缀
const hefengFreeApi = "https://devapi.qweather.com/v7"; //免费API前缀
...
const geoApi = "https://geoapi.qweather.com/v7/city/lookup?" //和风天气地理位置api
    ...
    wx.getLocation({
      ...
      success(res) {
        ...
        wx.request({
          ...
          //url: `${geoApi}location=101230112&key=${key}`,
          success(res) {
