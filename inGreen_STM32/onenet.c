...
#define AUTH_INFO	"inHome_zui_jiandan"

#define DEVID		"inHome"


extern unsigned char esp8266_buf[128];
extern u8 alarm;//是否报警的标志
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				else
				{
					...
					{
						json_value = cJSON_GetObjectItem(json , "value"); //获取JSON对象中的某个成员的值。
						if(json_value->valueint)  //json_value = 3
              LED0 = 0;//开灯
						else 
              LED0 = 1;//关灯
					}
            ...
            //下边这段比较容易频繁发生蜂鸣器的响声，实际运用并不佳。况且一个灯亮足够，无需那么注意
            /*
